...
const Input: React.FC<InputProps> = ({
  ...
  const validateInput = (value: string) => {
    ...
    // Si ha pasado todas las validaciones y hay un valor
    if (value) {
      setIsValid(true);
    }
  };
  ...
  useEffect(() => {
    // Validar el valor actual cuando cambia el valor o se establece touched
    if (touched) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Si ya ha sido tocado, validar en cada cambio
    if (touched) {
  ...
  return (
    ...
    >
      {label && (
        <label
          htmlFor={inputId}
          className={`block mb-1.5 font-medium ${labelSizeClasses[size]} ${
            disabled
